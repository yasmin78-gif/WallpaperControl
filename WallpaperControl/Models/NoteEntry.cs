namespace WallpaperControl
{
    ...
    internal sealed record NoteEntry
    {
        ...
        internal bool IsOverdue(DateTime now) => HasReminder && !IsCompleted && DueDate is DateOnly date
            && (date < DateOnly.FromDateTime(now) || (date == DateOnly.FromDateTime(now) && DueTime is TimeOnly time && time < TimeOnly.FromDateTime(now)));

        internal bool IsDueToday(DateTime now) => HasReminder && !IsCompleted && DueDate == DateOnly.FromDateTime(now)
    ...
    internal static class NoteGrouping
    {
        internal static IReadOnlyList<NoteGroup> Build(IEnumerable<NoteEntry> entries, DateTime now)
        {
            ...
            return entries.Where(e => !e.IsCompleted)
                .GroupBy(e => !e.HasReminder ? (0, (DateOnly?)null) : e.IsOverdue(now) ? (1, (DateOnly?)null) : (2, e.DueDate))
                ...
                .Select(g => new NoteGroup(g.Key.Item1 == 0 ? "NotesUndated" : g.Key.Item1 == 1 ? "NotesOverdue"
                    ...
                    g.Key.Item2, g.OrderBy(e => e.DueDate).ThenBy(e => e.DueTime.HasValue).ThenBy(e => e.DueTime)
