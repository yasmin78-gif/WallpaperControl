...
- `LayeredWidgetBitmap`: transparent bitmap upload for calendar, system, weather, and next-wallpaper widgets, including the matching native declarations and `finally` cleanup of temporary GDI handles. The caller still owns the bitmap.
- `NextWidgetRenderer`: Minimal, Clean, and Glow rendering plus hover feedback. Minimal preserves the original next button. Rendering is separate from click/drag handling; styles do not change its size or position.
