...
{
    internal readonly record struct NoteHit(Guid? Id, bool Checkbox, bool Add);
    ...
    internal sealed class NotesWidgetForm : Form
    {
        ...
        internal int OpenCount => store.Entries.Count(e => !e.IsCompleted);
        ...
        internal Bitmap RenderBitmap(int heightLimit = 2000, int dpi = 96)
        {
            ...
            float contentHeight = groups.Sum(g => 26 + g.Entries.Count * 48);
            ...
            try
            {
                ...
                foreach (NoteGroup group in groups)
                {
                    ...
                    {
                        RectangleF bounds = new(16, y, viewport.ContentBounds.Width - 4, 46);
                        if (bounds.Bottom >= viewport.ContentBounds.Top && bounds.Top < viewport.ContentBounds.Bottom)
                        {
                            rows.Add((entry, bounds));
                            DrawEntry(g, entry, bounds, now, font, paragraph, culture, palette.muted, palette.title, palette.text);
                        }
                        y += 48;
                    }
                }
                if (groups.Count == 0) g.DrawString(Localization.Get("NotesEmpty", language), font, muted, viewport.ContentBounds, paragraph);
                g.Restore(state);
        ...
        private static void DrawEntry(Graphics graphics, NoteEntry entry, RectangleF bounds, DateTime now,
            Font font, StringFormat paragraph, CultureInfo culture, Color muted, Color titleColor, Color textColor)
        {
            ...
            string prefix = overdue ? entry.DueDate?.ToString("d", culture) + "  " : "";
            if (entry.DueTime is TimeOnly time) prefix += time.ToString("t", culture) + "  ";
            string title = FitTitle(graphics, prefix + entry.Title, font, bounds.Width - 28, 38);
            Color color = overdue ? Color.FromArgb(245, 232, 193, 145) : entry.IsDueToday(now) ? titleColor : textColor;
            using SolidBrush ink = new(color);
            graphics.DrawString(title, font, ink, new RectangleF(42, bounds.Y + 3, bounds.Width - 28, 38), paragraph);
        }
        ...
        protected override void OnMouseUp(MouseEventArgs e)
        {
            ...
            if (pressed is NoteHit hit)
            {
                ...
                {
                    if (hit.Add) edit(null);
                    else if (hit.Id is Guid id)
                    {
                        if (hit.Checkbox)
                        {
                            if (!store.Complete(id, true)) MessageBox.Show(Localization.Get("NotesSaveError", settings.ClockLanguageCode), Text);
                        }
