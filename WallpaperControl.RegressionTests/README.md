...
Next-widget style checks cover all three styles, missing/invalid legacy values, cloned settings, live-preview callbacks, language changes, default reset, transparent corners, and hover rendering. Persistence uses a unique temporary registry key, passed explicitly to `WidgetSettings.Load/Save` and removed afterward; the production settings key is not written.
